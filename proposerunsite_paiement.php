...
elseif(!empty($_GET['erreur']) and is_numeric($_GET['erreur'])) {
	...
	if($sR['Etat'] == 4) {
		echo '<p>Vous ne payez pas le lien mais le temps que je vais passer à la modération de votre fiche.</p><p>Le fait de payer NE GARANTIT PAS par que votre site sera accepté. Si le contenu est dupliqué ou que votre site ne respecte pas les règles, nous n\'auront aucun scrupule à le refuser.</p><p>Le coût pour la modération est de '.$sRConfig['PaypalPrix'].' '.$sRConfig['PaypalMonnaie'].'.</p><p>Vous recevez un email pour vous signaler le résultat de la modération qui est faite en moins de 24 heures.</p><p>Les inscriptions non payées dans les 4 heures seront effacées automatiquement.</p><form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top"><input type="hidden" name="cmd" value="_s-xclick"><input type="hidden" name="hosted_button_id" value="WDYJT47ERXTBU"><input type="hidden" name="custom" value="'.$sR['KeyGen'].'" /><input type="image" src="https://www.paypalobjects.com/fr_FR/FR/i/btn/btn_buynowCC_LG.gif" border="0" name="submit" alt="PayPal, le réflexe sécurité pour payer en ligne"><img alt="" border="0" src="https://www.paypalobjects.com/fr_FR/i/scr/pixel.gif" width="1" height="1"></form>';
	}
	else {
		echo '<div class="alert alert-warning"><p>Votre fiche est en attente de modération.</p></div>';
	}

	echo '<h2>Votre fiche</h2><p><strong>Catégorie</strong> :: '.hs($aCategories[$sR['idCategorie']]['Titre']).'<br /><strong>Url</strong> :: '.hs($sR['Url']).'<br /><strong>Titre</strong> :: '.hs($sR['Titre']).'<br /><br /><strong>Description Courte</strong> :: '.nl2br(hs($sR['Description1'])).'<br /><br /><strong>Description Longue</strong> :: '.nl2br(hs($sR['Description2'])).'<br />';
}
