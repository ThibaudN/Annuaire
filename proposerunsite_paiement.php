...
elseif(!empty($_GET['erreur']) and is_numeric($_GET['erreur'])) {
	...
	if($sR['Etat'] == 4) {
		echo '<div class="row"><div class="col-lg-9 col-md-9 col-sm-12 col-xs-12"><p>Vous ne payez pas le lien mais le temps que je vais passer à la modération de votre fiche.</p><p>Le fait de payer NE GARANTIT PAS par que votre site sera accepté. Si le contenu est dupliqué ou que votre site ne respecte pas les règles, nous n\'auront aucun scrupule à le refuser.</p><p>Le coût pour la modération est de '.hs($sRConfig['PaypalPrix']).' '.hs($sRConfig['PaypalMonnaie']).'.</p><p>Vous recevez un email pour vous signaler le résultat de la modération qui est faite en moins de 24 heures.</p></div><div class="col-lg-3 col-md-3 col-sm-12 col-xs-12"><form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top"><input type="hidden" name="cmd" value="_s-xclick"><input type="hidden" name="hosted_button_id" value="'.hs($sRConfig['PaypalBoutonId']).'"><input type="hidden" name="custom" value="'.hs($sR['KeyGen']).'" /><input type="image" src="https://www.paypalobjects.com/fr_FR/FR/i/btn/btn_buynowCC_LG.gif" border="0" name="submit" alt="PayPal, le réflexe sécurité pour payer en ligne"><img alt="" border="0" src="https://www.paypalobjects.com/fr_FR/i/scr/pixel.gif" width="1" height="1"></form></div></div>';
	}
	else {
